...
total_accel_data = accel_plot %>% 
  group_by(week, day_number, day_of_the_week, hours) %>% 
  summarise(sum_activity=sum(activity)) %>% 
  arrange(week, day_number, day_of_the_week)

total_accel_data %>% 
  ggplot(aes(x= hours, y = sum_activity, fill = day_of_the_week)) + 
...
accel_plot %>% 
  ggplot(aes(x= (minute), y = day_number, fill = day_number)) + 
